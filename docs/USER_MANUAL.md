...
running on the **Xteink X4** (Espressif ESP32-C3): an 800×480 black-and-white
...
- **V3 devices** (most current Midea/PortaSplit units) need a **token and key**
  ...
  `V3` in the list; enter the credentials through the web portal (§9.4), then
...
- Networks are **added, edited, and deleted in the web portal** (§9.4) —
...
- **Weather** (if enabled, §9.3) — a line like `Berlin 21.3°C Cloudy 63%` plus
...
Shows the link state and opens the network list (§4).

### 9.2 Devices

Saved air conditioners and discovery (§3). Selecting a saved device and
...
account, no API key. Setup is done in the web portal (§9.4): a display name
...
Reboots into provisioning mode (§3) — the recovery path for forgotten or
...
Settings → Hotspot → re-provision (§3). This always works.
...
Wrong token/key. Re-extract them from the Midea cloud (see §3) and re-enter
