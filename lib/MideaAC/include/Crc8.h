#pragma once

// CRC-8 (854 table) checksumming the command payload + message id inside
// Midea AC frames. Table transcribed verbatim from msmart crc8.py at
// d7db53b; algorithm: init 0, per-byte table lookup, no final XOR.

#include <cstddef>
#include <cstdint>

namespace midea {

// inline keeps a single program-wide copy in flash (.rodata).
inline constexpr uint8_t kCrc8Table[256] = {
    };

// Mirrors msmart crc8.calculate(): crc = table[(crc ^ byte) & 0xFF], init 0.
constexpr uint8_t crc8(const uint8_t* data, std::size_t len) {
  uint8_t crc = 0;
  for (std::size_t i = 0; i < len; ++i)
    crc = kCrc8Table[(crc ^ data[i]) & 0xFF];
  return crc;
}

} // namespace midea
