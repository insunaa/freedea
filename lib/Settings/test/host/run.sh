#!/bin/sh
# Host-side unit tests for lib/Settings (pure C++, no Arduino/ESP-IDF).
# Compiles each test_*.cpp together with the settings core using the host
# compiler and runs the binaries. Exits non-zero if any build or run fails.
#
...
for src in $tests; do
  ...
  echo "== [build] $src"
  if $CXX $CXXFLAGS -o "$bin" "$src" ../../src/Settings.cpp; then
    echo "== [run] $bin"
    if "$bin"; then
      echo "== [pass] $bin"
    else
